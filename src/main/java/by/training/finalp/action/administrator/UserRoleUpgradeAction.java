...
public class UserRoleUpgradeAction extends Action {
    private static Logger logger = LogManager.getLogger(UserRoleUpgradeAction.class);
    ...
    public Forward exec(HttpServletRequest request, HttpServletResponse response) throws ServiceException {
        ...
        if (id != null) {
            ...
            if (user.getRole().ordinal() < 2 && identity != ((User) request.getSession().getAttribute("authorizedUser")).getId()) {
                user.setRole(Role.values()[user.getRole().ordinal() + 1]);
                userService.save(user);
                logger.info(String.format("User \"%s\" role was upgraded", user.getLogin()));
            }
        }
